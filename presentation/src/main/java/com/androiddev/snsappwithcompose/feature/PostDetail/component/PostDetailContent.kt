...
import com.androiddev.snsappwithcompose.common.component.paging.DefaultErrorView
...
fun PostDetailContent(
    ...
    val isRefreshing = isManualRefreshing && (
            pagingComments.loadState.refresh is LoadState.Loading || postDetailUiState.isLoading
            )

    LaunchedEffect(pagingComments.loadState.refresh) {
        //pullRefresh 처리
        if (isManualRefreshing &&
                pagingComments.loadState.refresh is LoadState.NotLoading && !postDetailUiState.isLoading
        ) {
        ...
        modifier = modifier
            ...
            .pullToRefresh(
                ...
                onRefresh = {
                    ...
                    pagingComments.refresh()
                    onRefresh()
                }
            )
    ) {
        if(pagingComments.loadState.refresh is LoadState.Error &&
            pagingComments.itemCount == 0 && postDetailUiState.errorMessage!=null) {
            isManualRefreshing = false

            DefaultErrorView(
                message = postDetailUiState.errorMessage.asString(),
                onRetry = {
                    onRefresh()
            ...
            LazyColumn(
                ...
                // 1. 게시글 본문 Header
                item {
                    if (postDetailUiState.isHeaderLoading && postDetailUiState.post == null) {
                        PostHeaderSkeleton()
                    } else {
